...
def eval_batch(image, labels, evaluator, index):
    ...
    elif args.method == 'gradient_rollout_cls_spec':
        Res = baselines.generate_gradient_att_cls_rollout(image.cuda(), start_layer=1, index=index).reshape(batch_size, 1, 14, 14)
    
    # segmentation test for the LRP baseline (this is full LRP, not partial)
    elif args.method == 'full_lrp':
        Res = orig_lrp.generate_LRP(image.cuda(), method="full", index=index).reshape(batch_size, 1, 224, 224)
    
    # segmentation test for our method
    elif args.method == 'transformer_attribution':
        Res = lrp.generate_LRP(image.cuda(), start_layer=1, index=index, method="transformer_attribution").reshape(batch_size, 1, 14, 14)
    
    # segmentation test for the partial LRP baseline (last attn layer)
    elif args.method == 'lrp_last_layer':
        Res = orig_lrp.generate_LRP(image.cuda(), index=index, method="last_layer", is_ablation=args.is_ablation)\
            .reshape(batch_size, 1, 14, 14)
    
    # segmentation test for the raw attention baseline (last attn layer)
    elif args.method == 'attn_last_layer':
        Res = orig_lrp.generate_LRP(image.cuda(), index=index, method="last_layer_attn", is_ablation=args.is_ablation)\
            .reshape(batch_size, 1, 14, 14)
    
    # segmentation test for the GradCam baseline (last attn layer)
    elif args.method == 'attn_gradcam':
        Res = baselines.generate_cam_attn(image.cuda(), index=index).reshape(batch_size, 1, 14, 14)
...
for batch_idx, (image, labels) in enumerate(iterator):
    ...
    if not np.isnan(pred).any():
        ...
        print("\n",f"Found NaN values in pred in batch: {batch_idx}", flush=True)
